...
rizu = Player("Mohammad Rizwan",Pakistan)
shaheen= Player("Shaheen Afridi",Pakistan)

kohli= Player("Virat Kohli",india)
rohit= Player("Rohit Sharma",india)
sirah= Player("Jasprit Bumrah",india)

shanaka= Player("Dasun Shanaka",srilanka)
rajapaksa= Player("M Rajapaksa",srilanka)
hasaranga= Player("Hasaranga",srilanka)
while True:
    ...
    if rand == 0:
        ...
        print(f"{cup.allTeams[tossWin].teamName} choose bowling and {cup.allTeams[tossLose].teamName} batting first")
        battingTeamObj = cup.allTeams[tossLose]
        bowlingTeamObj = cup.allTeams[tossWin]
    else:
        ## Winner Team Choose Batting
        print(f"{cup.allTeams[tossWin].teamName} choose batting first and {cup.allTeams[tossLose].teamName} bowling")
